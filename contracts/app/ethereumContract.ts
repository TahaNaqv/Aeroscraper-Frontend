...
export const getAppEthContract = (
  ...
  const { contractAddress, oraclecontractAddress, ausdContractAddress } =
    getContractAddressesByChain(chainName);
  ...
  const getVAA = async (): Promise<any> => {
    ...
    const priceIdByCLient: Record<
      ChainName,
      { priceId: string; serviceUrl: string }
    > = {
      [ChainName.SEI]: {
        priceId:
          "53614f1cb0c031d4af66c04cb9c756234adad0e1cee85303795091499a4084eb",
        serviceUrl: "https://xc-mainnet.pyth.network/",
      },
      [ChainName.ARCHWAY]: {
        priceId:
      ...
      [ChainName.NEUTRON]: {
        ...
        serviceUrl: "https://hermes-beta.pyth.network/",
      },
      [ChainName.INJECTIVE]: {
        priceId:
    ...
    const connection = new PriceServiceConnection(
      priceIdByCLient[chainName].serviceUrl,
      {
    ...
    const res = await connection.getLatestPriceFeeds([
      priceIdByCLient[chainName].priceId,
    ]);
  ...
  const getTotalCollateralAmount = async (): Promise<
  ...
  const getTrove = async (
    user_addr: string
  ): Promise<GetTroveResponse | undefined> => {
    if (chainName === ChainName.INJECTIVE) {
      const res = await chainGrpcWasmApi.fetchSmartContractState(
        contractAddress,
        toBase64({ trove: { user_addr } })
      );
      const data: any = fromBase64(res.data as any);
      return data;
    }
  ...
  const openTrove = async (
    ...
    if (chainName === ChainName.INJECTIVE) {
      ...
      const msg1 = MsgExecuteContractCompat.fromJSON({
        ...
        funds: [
          coin(
            getRequestAmount(amount, baseCoin.decimal),
            BaseCoinByChainName[chainName].denom
          ),
        ],
      });
  ...
  const borrowLoan = async (senderAddress: string, amount: number) => {
    ...
    if (chainName === ChainName.INJECTIVE) {
      ...
      const msg1 = MsgExecuteContractCompat.fromJSON({
        ...
        msg: {
          borrow_loan: {
            loan_amount: getRequestAmount(amount, baseCoin.ausdDecimal),
          },
        },
      });
