                    ...
                    Object.values(AppVersion).map((version, index, { length }) => {
                        return (
                            <>
                                <div
                                    key={version}
                                    className={`flex items-center gap-2 py-4 cursor-pointer transition ${version === selectedAppVersion ? 'opacity-100' : 'opacity-60 hover:opacity-80'}`}
                                    onClick={() => {
                                ...
                                {index < (length - 1) && <span className='w-full h-[1px] bg-white bg-opacity-100' />}
                            </>
                        )
                    })
                }
